...
public class Helper {
	...
	public Helper() {
		...
		long base = 1;
		// if the size is a power of two, the keys will be more evenly distributed across the array 
		// with minimal collision leading to better retrieval performance
		for (int i = 0; i <= 32; i++) {
	...
	public static long hashSocketAddress (InetSocketAddress addr) {
		int i = addr.hashCode(); // hashcode() Returns the hashcode for this InetSocketAddress object
		// System.out.println("HashCode: = " + i); // -1062660547
	...
	public static InetSocketAddress createSocketAddress (String addr) {
		...
		if (splitted.length >= 2) {
			...
			try {
				m_ip = InetAddress.getByName(ip); // getname() returns the instance of InetAddress containing IP and Host name of host represented by host argument.
